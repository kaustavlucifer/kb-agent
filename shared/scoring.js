...
title: ≤60 chars, keywords front-loaded (searchable feature/task names first), no question format, matches article intent. Describes the ISSUE BEHAVIOR, not the solution. A title is compliant as long as it includes AT LEAST ONE of: product name, cloud, audience, or mode — dock only titles with NONE of these contextual identifiers (e.g. "How to fix the error", "Troubleshooting login issues" have no product, cloud, audience, or mode and are non-compliant). GOOD: "Lead History report displays no results", "Create a Case Assignment Rule for Customer Portal", "Salesforce.org Trial Extensions for Nonprofits". BAD: "Enabling Field History Tracking to populate a report" (states the solution), "Troubleshooting Flows" (not product-specific), "How to use Assignment Rules to automatically assign a Case..." (keyword buried, over-length).

summary: ≤170 chars, uses DIFFERENT words than title (distinct, not a restatement), includes synonyms/keywords not in title, states the intent — WHAT problem/question the article resolves and WHY — and includes exact error text for error articles. Does NOT name a target audience or role ("for developers", "scoped for admins/architects" is jargon and should be docked). GOOD (title "Disable Chatter" → summary "Turn off Chatter features across an entire Salesforce org to reduce notification noise."). The summary is shown to customers in help-site search AND used by Agentforce for retrieval, so it must add searchable terms and carry the intent statement, not echo the title.
...
content: The intent statement (WHAT question/problem is resolved and WHY) belongs in the Summary — the Description itself should open directly with the problem, symptom, or observed behavior, NOT with a meta sentence like "This article addresses…"/"This article explains…"/"This article is scoped for…" (dock such openers). Description explains WHY the issue happens (root-cause context, when/where/how it occurs). Resolution opens with a summary paragraph stating what the steps accomplish and the intent behind them — never jump straight into steps assuming the Description is enough. Most important information first (readers scan in an "F" shape and drop off). Real-life Salesforce examples with realistic-format data (not xxxxx placeholders) when the topic is complex or confusing — but do NOT dock step-by-step procedural walkthroughs, concrete error messages, or reproduction steps for lacking "scenario" framing; these already count as compliant real-life context. Every acronym, synonym, and abbreviation explained on first use EXCEPT standard Salesforce platform terminology, which needs no definition: universally known abbreviations (API, CRM, URL, etc.), Salesforce-standard terms (Lightning Experience, Classic, org, etc.), programming-language keywords/syntax tokens (SQL, HTML, SELECT, etc.), HTML tags appearing in the text body (BR, P, DIV, SPAN, IMG, TABLE, etc.), and common English emphasis words in caps (NOTE, TIP, WARNING, CAUTION, NEW). DO dock genuinely undefined domain-specific acronyms and currency/unit codes (e.g. "PHP", "USD" used without stating "PHP (Philippine Peso)"/"USD (United States Dollar)" on first use). If the product name is not explicitly clear in the content, it must be stated alongside the term (e.g. "MuleSoft APIs" not just "APIs"). Simple present tense. Written in impersonal, product-facing voice — avoid first/second-person pronouns ("you", "your", "we", "our"), prefer imperative for steps and noun/passive phrasing for descriptions; do NOT address the reader directly or reference "the customer" (dock pronoun-heavy, reader-addressed prose). Complete problem/environment/cause/resolution. Original content, not copied from external sources. No speculative statements. Each section self-contained (readable in isolation as a chunk).

grammar: All prose is composed of grammatically complete sentences or grammatically valid constructions — no sentence fragments (a construction lacking both a subject and a main verb, or a sentence truncated mid-thought). The following ARE COMPLETE and must NOT be flagged as fragments: imperative sentences (instructions/commands), numbered procedural steps written in imperative form, and bulleted list items that are noun phrases used as labels, menu options, or feature names. Only dock genuine fragments — truncated clauses, dangling modifiers, missing verbs — not terse-but-valid procedural or list writing.
...
links: LINK HYGIENE — minimize references and links; an article is NOT link-heavy with 4 or fewer anchored hyperlinks (<a href> tags) total, but 5 or more ANCHORED hyperlinks (see ANCHORED count in the user message) makes it link-heavy and non-compliant — dock hard and note the exact count. Within that budget: descriptive hyperlink TEXT, never raw exposed URLs ("See 'Postmaster Tools' on Gmail Support" not the bare URL; name the external site). No internal-only URLs (orgcs.lightning.force.com, help URLs behind login). No broken links. Article is self-contained — links supplement, they must never replace content or offload the actual answer to another page. Do NOT reward an article for having many links or an extensive Additional Resources section; a small number of well-chosen citations is the ceiling.
...
export async function fetchArticleBodies(articleIds, session) {
  ...
  const runBatchWithRetry = async (batch) => {
    try {
